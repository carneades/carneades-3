...
(library
 ...
 (define (description->rules desc)
   (let* ((rdf-id ((sxpath "@rdf:ID/text()" namespaces) desc))
          ...
          (prop-value-rules (map (lambda (p) (property-value->rule individual-name p)) prop-values)))
     (newline)
     (display "description (")
 ...
 (define (member-type->rule individual-name type)
   ...
   (let* ((rdf-resource ((sxpath "@rdf:resource/text()" namespaces) type))
          ...
          (rule-head (cond ((not (null? rdf-resource)) (list (string->symbol (text->name rdf-resource))
                                                             individual-name))
                           ((not (null? owl-classes)) (class-description->formula individual-name (car owl-classes)))
                           ((not (null? restrictions)) (class-restriction->formula individual-name (car restrictions)))
 ...
 (define (property-value->rule individual-name property-value)
