...
(library 
 ...
 (define (instantiate-argument arg subs)
   (make-argument (argument-id arg)
                      ...
                      (argument-weight arg)
                      (argument-direction arg)
                      (apply-substitution subs (argument-conclusion arg))
                      (map (lambda (p) 
                             (cond ((ordinary-premise? p)
                                    (make-ordinary-premise (apply-substitution subs (premise-atom p))
                                                           (premise-polarity p)
                                                           (premise-role p)))
                                   ((exception? p)
                                    (make-exception (apply-substitution subs (premise-atom p))
                                                    (premise-polarity p)
                                                    (premise-role p)))
                                   ((assumption? p)
                                    (make-assumption (apply-substitution subs (premise-atom p))
                                                     (premise-polarity p)
                                                     (premise-role p)))))
                           (argument-premises arg))
                      (argument-scheme arg)))
