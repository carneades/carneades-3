...
;;
;; Modified by Derick Eddington to be included into the (carneades srfi time) R6RS library.
    ...
    [(assoc type tm:time-error-types)
     ...
     (lambda (p)
       (if value
         ...
         (error caller (cdr p))))]
    [else
     (error caller "(library (carneades srfi time)) internal error: unsupported error type" type)]))
