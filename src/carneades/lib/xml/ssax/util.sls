(library 
  ...
  (define (any? <pred?> coll)
	...
	(let loop ((i 0))
	  (if (>= i len) #f
	      (or (<pred?> (vector-ref coll i)) (loop (add1 i)))))))

     ((string? coll)
      (let ((len (string-length coll)))
	(let loop ((i 0))
	  (if (>= i len) #f
	      (or (<pred?> (string-ref coll i)) (loop (add1 i)))))))
  ...
  (define (string->integer str start end)
    (and (< -1 start end (add1 (string-length str)))
	 (let loop ((pos start) (accum 0))
	   (cond
	    ((>= pos end) accum)
	    ((char-numeric? (string-ref str pos))
	     (loop (add1 pos) (+ (char->integer (string-ref str pos)) 
		...
		(cond
		 ...
		 (else
		  (let ((quoted-char
			...
			(new-to 
			 (index-cset str (add1 to) bad-chars)))
		    (if (< from to)
			(cons
			 (substring str from to)
			 (cons quoted-char (loop (add1 to) new-to)))
			(cons quoted-char (loop (add1 to) new-to))))))))))))
  )