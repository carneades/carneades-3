...
(define-syntax define-def
  (syntax-rules ()
    ((_ ident alist default)
     (define ident
       (or (cond
            ((assq 'ident alist) => cdr)
            (else #f))
           default)))))
...
(define (http-transaction req-method req-url req-parms response-handler)
  ...
  (define-def logger     req-parms
    (lambda (port msg . other-msgs) (display msg) (display other-msgs)))
  ...
  (define (make-req schema dummy host resource)
    (let* ((target-host (or http-proxy host))
	   ...
	   (socket (make-client-socket target-host-proper target-port))
	   (http-i-port (transcoded-port (socket-input-port socket) (native-transcoder)))
	   (http-o-port (transcoded-port (socket-output-port socket) (native-transcoder))))
