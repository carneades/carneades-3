...
(library
 ...
 (export
  shtml-entity->char-data
  sxml->html sxml->html-noindent
  sxml->string sxml->xml)
 ...
 (import 
  ...
  (only (rnrs unicode)
	string-ci=?)
  (only (rnrs lists)
	assq assoc assv filter memv memq)
  (only (rnrs io ports)
	put-string
	open-file-output-port output-port? close-port))

 ;; `map' and `append' in a single pass:
 ;; (map-append func lst) = (apply append (map func lst))
 ;; A simplified analogue of `map-union' from "sxpathlib.scm"
 (define (map-append func lst)
   (if (null? lst)
      lst
      (append (func (car lst))
	      (map-append func (cdr lst)))))

 ;; procedure apply-string-append :: STR-LST -> STRING
 ;; str-lst ::= (listof string)
 ;; Concatenates `str-lst' members into a single string
 ;; (apply-string-append str-lst) = (apply string-append str-lst)

 (define (apply-string-append str-lst)
   (apply string-append str-lst))

 ;; Analogue of `assoc'
 ;; However, search is performed by `cdr' of each alist member and `string=?' is
 ;; used for comparison
 (define (assoc-cdr-string= item alist)
   (cond
    ((null? alist) #f)
    ((string=? (cdar alist) item) (car alist))
    (else (assoc-cdr-string= item (cdr alist)))))

 ;; Analogue of `member' for strings that uses case insensitive comparison
 (define (member-ci str lst)
   (cond
    ((null? lst) #f)
    ((string-ci=? str (car lst)) lst)
    (else (member-ci str (cdr lst)))))

 ;; Analogue of `member'
 ;; The end of the `lst' is returned, from the first member that satisfies
 ;; the `pred?'
 (define (mem-pred pred? lst)
   (cond
    ((null? lst) #f)
    ((pred? (car lst)) lst)
    (else (mem-pred pred? (cdr lst)))))

 ;;-------------------------------------------------
 ;; Borrowed from "char-encoding.scm"

 (define char-nl (integer->char 10))

 ;; A string consisting of a single newline character
 (define newline-str (string char-nl))

 ;;-------------------------------------------------
 ;; Borrowed from "sxpathlib.scm"

 ;; A simplified implementation of `select-kids' is sufficient for the serializer
 (define (select-kids test-pred?)
   (lambda (node)		;; node or node-set
     (cond 
      ...
      (else
       (map-append (select-kids test-pred?) node)))))
 ...
 (define (clean-fragments fragments)
   (reverse (let loop ((fragments fragments) (result '()))
	      (cond
	       ((null? fragments) result)
	       ((null? (car fragments)) 
		(loop (cdr fragments) result))
	       ((pair? (car fragments))
		(loop (cdr fragments) 
 ...
 (define (display-fragments-2nesting fragments-level2 port)
   (for-each
    (lambda (level1)
      (if (pair? level1)
	 (for-each (lambda (x) (put-string port x))
		   level1)
	 (put-string port level1)))
    fragments-level2))
 ...
 (define conventional-ns-prefixes
 ...
 (define (namespace-assoc-for-elem elem)
   ((select-kids (lambda (node) (pair? node)))
    ((select-kids
      (lambda (node) (and (pair? node) (eq? (car node) '*NAMESPACES*))))
     (append
      ((select-kids  ;; compatibility with SXML 3.0
	(lambda (node) (and (pair? node) (eq? (car node) '*))))
       ((select-kids
	 (lambda (node) (and (pair? node) (eq? (car node) '*))))
	elem))
      ((select-kids  ;; compatibility with SXML 2.X
	(lambda (node) (and (pair? node) (eq? (car node) '**))))
       elem)))))

 ;; Returns (listof <namespace-assoc>) for the SXML document node
 (define (ns-assoc-for-top doc)
   ((select-kids (lambda (node) (pair? node)))
    ((select-kids
      (lambda (node) (and (pair? node) (eq? (car node) '*NAMESPACES*))))
     ((select-kids
 ...
 (define (extract-original-prefix-binding namespace-assoc-lst)
   (map
 ...
 (define (update-space-specifier elem space-preserve?)
   (let ((xml-space-val ((select-kids string?)
		       ((select-kids (lambda (node) (and (pair? node) (eq? (car node) 'xml:space))))
			((select-kids (lambda (node) (and (pair? node) (eq? (car node) '*))))
			 elem)))))
     (cond
 ...
 (define (normalize-sequence node-or-sequence)
   (letrec
       ((normaliz-step-1
	...
	(normaliz-step-2
	 ...
	 (lambda (seq)
	   (map
	    (lambda (item) (atomic->string item))
	    seq)))
	(normaliz-step-3
 ...
 (define (string->cdata-section str)
   (let ((flush-buffer
 ...
 (define escape-alist-char-data
   '((#\& . "&amp;") (#\< . "&lt;") (#\> . "&gt;")))
 (define escape-alist-att-value
   (append `((#\' . "&apos;") (#\" . "&quot;")
	     ;; Escaping the newline character in attribute value
	     (,char-nl . "&#10;"))
	   escape-alist-char-data))
 (define escape-alist-html-att
   '((#\& . "&amp;") (#\> . "&gt;") (#\' . "&apos;") (#\" . "&quot;")))

 ;; Escape a string with the `xml-char-escaped' and with the `escape-alist'
 ;; supplied
 ;; escape-alist ::= (listof (cons char string))
 ;; html-method? ::= #t | #f
 ;; Returns the escaped string
 (define (string->escaped str escape-alist html-method?)
   (let loop ((src (string->list str))
	    (adj-chars '())
	    (res '()))
     (cond
 ...
 (define (string->char-data str)
   (string->escaped str escape-alist-char-data #f))

 (define (string->att-value str)
   (string->escaped str escape-alist-att-value #f))

 (define (string->html-att str)
   (string->escaped str escape-alist-html-att #t))
 ...
 (define (shtml-entity->char-data entity)
 ...
 (define (qname->string prefix-string local-part)
   (if prefix-string
 ...
 ;; method ::= 'xml | 'html
 (define (attribute->str-lst prefix-string local-part att-value method)
   (let ((attval (atomic->string att-value)))
     (cond
      (prefix-string
       (list " " prefix-string ":" local-part "=\""
	     ((if (eq? method 'html)
		 string->html-att
		 string->att-value) attval)
	     "\""))
      ((eq? method 'html)
       (if (string=? local-part attval)  ;; boolean attribute
	  (list " " local-part)
	  (list " " local-part "=\"" (string->html-att attval) "\"")))
      (else  ;; unprefixed attribute, XML output method
       (list " " local-part "=\"" (string->att-value attval) "\"")))))
 ...
 ;;  issue
 (define (namespace-decl->str-lst prefix-string namespace-uri)
   (list " xmlns:" prefix-string "=\""
	 (string->att-value namespace-uri) "\""))
 ...
 (define (comment->str-lst comment-node)
 ...
 (define (processing-instruction->str-lst pi-node method)
   ...
   (let ((use-ns-id-or-generate-prefix
	(lambda (ns-id)
	 ...
	 (lambda (namespace-uri ns-id)
	   (cond
	    ((assoc-cdr-string= namespace-uri declared-ns-prefixes)
	     => (lambda (pair)
	    ...
	    (else  ;; namespace undeclared
	     (values
	      (cond
	       ((assoc-cdr-string= namespace-uri ns-prefix-assig)
		=> (lambda (pair)
 ...
 (define construct-start-end-tags 
   (lambda (elem method ns-prefix-assig namespace-assoc declared-ns-prefixes)
     (let ((ns-assoc-here (namespace-assoc-for-elem elem))
	 (empty? (empty-elem? elem)))
       (let ((ns-prefix-assig (append (extract-original-prefix-binding ns-assoc-here)
				    ns-prefix-assig))
	   (namespace-assoc (append ns-assoc-here namespace-assoc)))
	 
	 (call-with-values
	     (lambda ()           
	       (name->qname-components  ;; element name
		(car elem) ns-prefix-assig namespace-assoc declared-ns-prefixes))
	   
	   (lambda (elem-prefix elem-uri elem-local elem-decl-required?)
	     (let loop ((attrs (reverse ((select-kids (lambda (node)  ;; not SXML 3.0 aux-list
						      (and (pair? node) (not (eq? (car node) '*)))))
				       ((select-kids (lambda (node)
						       (and (pair? node) (eq? (car node) '*))))
					elem))))
		      (start-tag (if (or (not empty?)
				       (and (eq? method 'html)
					  (not elem-prefix)
					  (member-ci elem-local
		      ...
		      (declared-ns-prefixes (if elem-decl-required? 		      ;; As if element namespace already declared
					       (cons (cons elem-prefix elem-uri)
						     declared-ns-prefixes)
					       declared-ns-prefixes)))
	       
	       (if (null? attrs)  ;; attributes scanned
		  (let ((elem-name (qname->string elem-prefix elem-local)))
		    (values (cons "<"
				  (cons elem-name
					(if
					 elem-decl-required?
					 (cons
					  (namespace-decl->str-lst elem-prefix elem-uri)
					  start-tag)
					 start-tag)))
			    (if empty? #f (list "</" elem-name ">"))
			    ns-prefix-assig
			    namespace-assoc
			    declared-ns-prefixes))
		  (call-with-values
		      (lambda ()
			(name->qname-components
			...
			(loop (cdr attrs)
				 ...
				 (cons (namespace-decl->str-lst attr-prefix attr-uri)
				       start-tag)
				 start-tag)
			      ns-prefix-assig
			      namespace-assoc
 ...
 (define node->nested-str-lst-recursive
   (lambda (node method ns-prefix-assig namespace-assoc declared-ns-prefixes
	    indentation space-preserve? cdata-section-elements text-node-handler)
     (if (not (pair? node))  ;; text node
	(text-node-handler (atomic->string node))
	(case (car node)  ;; node name
	  ((*COMMENT*)
	   (comment->str-lst node))     
	  ((*PI*)
	   (processing-instruction->str-lst node method))
	  ((&)
	   (shtml-entity->char-data node))
	  ((*DECL*)  ;; recovering for non-SXML nodes
	   '())
	  (else  ;; otherwise - an element node
	   (call-with-values
	       (lambda ()
		 (construct-start-end-tags node 
					   method
					   ns-prefix-assig 
					   namespace-assoc 
					   declared-ns-prefixes))	    
	     (lambda (start-tag end-tag ns-prefix-assig namespace-assoc declared-ns-prefixes)	       	       
	       (if (not end-tag)  ;; empty element => recursion stops
		  start-tag
		  (let ((space-preserve?
		       (update-space-specifier node space-preserve?))
		      (text-node-handler
		       (cond
			((memq (car node) cdata-section-elements)
			 string->cdata-section)
			((and (eq? method 'html)
			    (member-ci (symbol->string (car node))
				       '("script" "style")))
			...
			(if indent-here
			   (append start-tag
				   (map (lambda (kid)
					  (list indent-here
						(node->nested-str-lst-recursive kid 
										method
										ns-prefix-assig 
										namespace-assoc 
										declared-ns-prefixes
										...
										text-node-handler)))
					content)
				   (cons newline-str (cons (cdr indentation) end-tag)))
			   (append start-tag
				   (map (lambda (kid)
					  (node->nested-str-lst-recursive
					   kid method
					   ns-prefix-assig namespace-assoc declared-ns-prefixes
 ...
 (define (display-node-out-recursive
     node port method
     ns-prefix-assig namespace-assoc declared-ns-prefixes
   ...
   (if
    ...
    (put-string port (text-node-handler (atomic->string node)))
    (case (car node)  ;; node name
      ((*COMMENT*)
       (for-each
	(lambda (x) (put-string port x))
	(comment->str-lst node)))
      ((*PI*)
       (for-each
	(lambda (x) (put-string port x))
	(processing-instruction->str-lst node method)))
      ((&)
       (put-string port (shtml-entity->char-data node)))
      ((*DECL*)  ;; recovering for non-SXML nodes
       #f)
      (else  ;; otherwise - an element node
       (call-with-values
	 ...
	 (lambda (start-tag end-tag
		       ...
		       declared-ns-prefixes)
	   (begin
	     (display-fragments-2nesting start-tag port)
	     (if end-tag  ;; there exists content
		(let ((space-preserve?
		     (update-space-specifier node space-preserve?))
		    (text-node-handler
		     (cond
 ...
 (define (make-xml-decl version standalone)
 ...
 (define (top->nested-str-lst doc
			 cdata-section-elements indent
			 method ns-prefix-assig
			 omit-xml-declaration? standalone version)
   (let* ((namespace-assoc (ns-assoc-for-top doc))
	(ns-prefix-assig (append
			  (extract-original-prefix-binding namespace-assoc)
			  ns-prefix-assig))
	(serialized-content (map (if indent  ;; => output each member from the newline
					...
					(list newline-str
					      (node->nested-str-lst-recursive
					       ...
					       indentation #f
					       cdata-section-elements string->char-data))))
				    (lambda (kid)
				      (node->nested-str-lst-recursive
				       kid method
				       ns-prefix-assig namespace-assoc '()
				       indent #f
				       cdata-section-elements string->char-data)))
				 ((select-kids  ;; document node content
				   (lambda (node)    ;; TODO: support SXML entities
				     (not (and
	...
	(if (and indent (not (null? serialized-content)))
	   ...
	   serialized-content)
	(list (make-xml-decl version standalone) serialized-content))))

 (define (display-top-out doc port
		     cdata-section-elements indent
		     method ns-prefix-assig
		     omit-xml-declaration? standalone version)
   (let ((no-xml-decl?  ;; no XML declaration was displayed?
	(if (not (or (eq? method 'html) omit-xml-declaration?))
	   (begin
	     (for-each  ;; display xml declaration
	      (lambda (x) (put-string port x))
	      (make-xml-decl version standalone))
	     #f)
	   #t))
	...
	((select-kids
	  (lambda (node)  ;; TODO: support SXML entities
		...
		(pair? node) (memq (car node) '(* ** *ENTITY*))))))
	 doc))
       (namespace-assoc (ns-assoc-for-top doc)))
     (let ((ns-prefix-assig
	  (append
	   (extract-original-prefix-binding namespace-assoc)
	   ns-prefix-assig)))
 ...
 ;; additional serialization parameters, the functions `sxml->string' and
 ;; `display-sxml' specified in this subsections may have a different number
 ...
 (define (sxml->string sxml-obj
		  cdata-section-elements indent
		  method ns-prefix-assig
		  omit-xml-declaration? standalone version)
   (let ((indent (if (and indent 
		     (not (string? indent)))
		  "  "
		  indent)))
     (apply-string-append (clean-fragments 
			   (top->nested-str-lst (normalize-sequence sxml-obj)
						cdata-section-elements indent
						method ns-prefix-assig
						omit-xml-declaration? standalone version)))))
 ...
 ;;------------------------------------------------------------------------------
 (define (display-sxml sxml-obj port-or-filename
		  cdata-section-elements indent
		  method ns-prefix-assig
		  omit-xml-declaration? standalone version)
   (if (string? port-or-filename)  ;; a filename?
      (let ((out (open-file-output-port port-or-filename)))
	(begin
	  (display-top-out (normalize-sequence sxml-obj) out
			   cdata-section-elements
			   (if (and indent (not (string? indent)))
			      "  " indent)
			   method ns-prefix-assig
			   omit-xml-declaration? standalone version)
	  (put-string out newline-str)  ;; newline at the end of file
	  (close-port out)))
      (display-top-out (normalize-sequence sxml-obj) port-or-filename
		       cdata-section-elements
 ...
 ;;
 ;; procedure parameterizable :: SXML-OBJ [PORT] {PARAM}* ->
 ;;                                    -> STRING|unspecified
 ...
 (define (parameterizable sxml-obj . port-or-filename+params)
   (call-with-values
       (lambda ()
     ...
     (lambda (port-or-filename params)
       (let loop ((params params)
		...
		(indent "  ")
		(method 'xml)
		(ns-prefix-assig conventional-ns-prefixes)
	 ...
	 (cond
	  ((null? params)  ;; all parameters parsed
	   (if port-or-filename
	      (display-sxml sxml-obj port-or-filename
			    cdata-section-elements indent
			    method ns-prefix-assig
			    omit-xml-declaration? standalone version)
	      (sxml->string sxml-obj
			    cdata-section-elements indent
			    method ns-prefix-assig
			    omit-xml-declaration? standalone version)))
	   ...
	   (let ((prm-value (cdar params)))
		...
		(loop (cdr params)
		      ...
		      (if (and (list? prm-value)
			    (not (mem-pred  ;; no non-pair members
				(lambda (x) (not (pair? x)))
				prm-value)))
 ...
 ;; 										  ;;
 ;; procedure sxml->xml :: SXML-OBJ [PORT-OR-FILENAME] -> STRING|unspecified	  ;;
 ;; 										  ;;
 ...
 (define (sxml->xml sxml-obj . port-or-filename)
   (if (null? port-or-filename)
      (sxml->string sxml-obj '() #t 'xml
		    conventional-ns-prefixes #t 'omit "1.0")
      (display-sxml sxml-obj (car port-or-filename) '() #t 'xml
		    conventional-ns-prefixes #t 'omit "1.0")))

 ;;------------------------------------------------------------------------------
 ;; procedure sxml->xml-noindent :: SXML-OBJ [PORT-OR-FILENAME] ->
 ;;                                      -> STRING|unspecified
 ;;
 ;; Serializes the `sxml-obj' into XML, without indentation.
 ;;------------------------------------------------------------------------------

 (define (sxml->xml-noindent sxml-obj . port-or-filename)
   (if (null? port-or-filename)
      (sxml->string sxml-obj '() #f 'xml
		    conventional-ns-prefixes #t 'omit "1.0")
      (display-sxml sxml-obj (car port-or-filename) '() #f 'xml
		    conventional-ns-prefixes #t 'omit "1.0")))

 ;;------------------------------------------------------------------------------
 ;; procedure sxml->html :: SXML-OBJ [PORT-OR-FILENAME] -> STRING|unspecified
 ;;
 ;; Serializes the `sxml-obj' into HTML, with indentation to facilitate
 ;; readability by a human.
 ;;
 ;; sxml-obj - an SXML object (a node or a nodeset) to be serialized
 ;; port-or-filename - an output port or an output file name, an optional argument
 ;;
 ;; If `port-or-filename' is not supplied, the functions return a string that
 ;; contains the serialized representation of the `sxml-obj'.
 ;;
 ...
 (define (sxml->html sxml-obj . port-or-filename)
   (if (null? port-or-filename)
      (sxml->string sxml-obj '() #t 'html '() #t 'omit "4.0")
      (display-sxml sxml-obj (car port-or-filename)
		    '() #t 'html '() #t 'omit "4.0")))

 ;;------------------------------------------------------------------------------
 ;; procedure sxml->html-noindent :: SXML-OBJ [PORT-OR-FILENAME] ->
 ;;                                       -> STRING|unspecified
 ;;
 ;; Serializes the `sxml-obj' into HTML, without indentation.
 ;;------------------------------------------------------------------------------
 (define (sxml->html-noindent sxml-obj . port-or-filename)
   (if (null? port-or-filename)
      (sxml->string sxml-obj '() #f 'html '() #t 'omit "4.0")
      (display-sxml sxml-obj (car port-or-filename)
		    '() #f 'html '() #t 'omit "4.0")))
