...
(library
 ...
 (export txp:ast-operation-helper
         txp:ast-params
         txp:ast-res
         txp:ast-api-helper
         txp:xpath->ast
 ...
 (import (rnrs base)
         (rnrs lists)
         (only (carneades lib xml ssax extra myenv) cerr)
         (carneades lib xml sxml xpath-parser)
         (only (carneades lib prelude) and-let*))
        ...
        (cond
          ...
          ((procedure? (car path))
           (loop (cons (list 'lambda-step (car path))
                       ast-steps)
                 (cdr path)))
          ((assq (car path) '((// . descendant-or-self) ((string->symbol "..") . parent)))          
           => (lambda (pair)
                (loop (cons
