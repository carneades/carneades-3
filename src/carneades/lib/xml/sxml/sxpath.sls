(library
  ...
  (import
   ...
   (only (carneades lib  xml sxml sxpathlib)
         as-nodeset map-union
	 ntype?? node-pos node-self 
	 node-equal? node-eq? nodeset?
	 ntype-namespace-id?? ntype-names??
         select-kids sxml:attribute 
	 sxml:complement sxml:filter sxml:node?);; sxml:   
   (only (carneades lib  xml sxml sxpath-ext)
         sxml:descendant sxml:descendant-or-self)
   (only (carneades lib  xml sxml txpath)
         sxml:xpath-expr))
