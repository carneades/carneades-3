(library
  ...
  (export
   ...
   sxml:namespace sxml:number
   sxml:not-equal?
   sxml:preceding sxml:preceding-sibling
   sxml:relational-cmp sxml:id sxml:string sxml:string-value)
  ...
  (import
   ...
   (only (carneades lib xml sxml sxml-tools)
         sxml:lookup sxml:ns-list)
   (only (carneades lib xml sxml sxpathlib)
         sxml:attribute sxml:attr-list sxml:child sxml:element? sxml:filter sxml:node?
         map-union nodeset? ntype??))
  ...
  ;; Rewritten to use R6RS open-string-input-port
  (define (sxml:number obj)
    (cond
     ((number? obj) obj)
     ((string? obj)
      (let ((nmb (open-string-input-port obj read)))
        (if (number? nmb)
            nmb
            0))) ;; NaN
     ((boolean? obj) (if obj 1 0))
