(library
  ...
  (import
   ...
   (only (rnrs lists)
         assq assoc)
   (only (rnrs r5rs)
         inexact->exact remainder)
   (rnrs io simple) ;; debugging only, delete this.
   (only (rnrs unicode)
         string-ci=?)
   (only (carneades lib  sys system)
	 exit)
   (only (carneades lib  types strings)
         string-index-right string-prefix? string-prefix-ci?)
   (only (carneades lib  xml sxml env)
         cerr nl)
   (only (carneades lib  xml ssax util)
         string-split substring?)
   (only (carneades lib  xml sxml sxml-tools)
         sxml:lookup)
   (only (carneades lib  xml sxml xpath-parser)
         sxml:whitespace
         txp:parameterize-parser txp:error? txp:signal-semantic-error)
   (only (carneades lib  xml sxml sxpathlib)
         as-nodeset ntype?? map-union nodeset? node-pos ntype-namespace-id??
         sxml:attribute sxml:child sxml:node? sxml:filter sxml:parent)
   (only (carneades lib  xml sxml sxpath-ext)
            sxml:ancestor sxml:ancestor-or-self
            sxml:boolean
            sxml:descendant sxml:descendant-or-self
            sxml:equal?
            sxml:following sxml:following-sibling
            sxml:id
            sxml:namespace sxml:not-equal?
            sxml:number sxml:preceding  sxml:preceding-sibling
            sxml:relational-cmp  sxml:string sxml:string-value))
