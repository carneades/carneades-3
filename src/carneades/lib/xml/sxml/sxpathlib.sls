(library
  ...
  (export
   as-nodeset
   sxml:attribute sxml:attr-list sxml:complement sxml:child
   sxml:element? sxml:filter ntype-namespace-id?? sxml:node? sxml:parent
   map-union ntype?? node-pos nodeset? ntype-names?? node-self node-eq? node-equal?
   select-kids)
