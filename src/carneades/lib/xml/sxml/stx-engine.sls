(library
  (carneades lib  xml sxml stx-engine)
  ...
  (export)
   ...
   (import
    ...
    (only (rnrs control)
	  when)
    (rnrs eval)
    (only (carneades lib  xml ssax ssax)
	  xml->sxml
	  ssax:read-attributes
	  ssax:read-markup-token
	  ssax:skip-pi ssax:skip-S)
    (only (carneades lib  xml sxml libmisc)
	  tee-2 tee-3 tee-4 verb-2
          lambda-tuple self)
    ...
    (only (carneades lib  xml sxml sxml-tools)
	  ...
	  sxml:node-parent
	  sxml:sxml->html
          sxml:attr sxml:attr-from-list sxml:content sxml:name->ns-id sxml:ncname sxml:string->html sxml:sxml->xml)
    (only (carneades lib  xml sxml sxpath)
	  if-car-sxpath
          sxpath)
    (only (carneades lib  io print)
	  pretty-print)
    ...
    (only (carneades lib  xml sxml env)
	  cerr nl)
    (primitives call-with-input-string))
   ...
   (define (stx:write-ss t+e fname)
     (let* ((of
       ...
       (for-each
        (lambda(x)
          (wrt nl "(list ") 
          (pretty-print (car x) of)
          (wrt "") 
          (pretty-print (cadr x) of)
          (wrt ")" nl))
        (car t+e))
...
;;;
   ;; This was initially a define-macro which generaates new symbols.
   ;; FIXME RPR - may need to come back and rewrite as a syntax-case macro.
   ;;
   (define-syntax stx:call-function
     (syntax-rules ()
       ((_ name type tpl-node $-env)
        (let ((fn ($-env 
                   (string->symbol name))))
          (if 
           (eq? fn '*LT-NOT-FOUND*)
