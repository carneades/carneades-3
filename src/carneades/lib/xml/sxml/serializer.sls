		  ...
		  (let ((space-preserve?
			...
			(lambda ()
			  (cond
			   ...
			   (else
			    (values (cons newline indentation)
				   ...
				   (map (lambda (kid)
					  (list indent-here
						(node->nested-str-lst-recursive kid 
										...
										text-node-handler)))
					content)
				   (cons newline (cons (cdr indentation) end-tag)))
			   (append start-tag
				   (map (lambda (kid)
   ...
   (if (string? port-or-filename)  ;; a filename?
	...
	(begin
	  (display-top-out (normalize-sequence sxml-obj) out
			   ...
			   method ns-prefix-assig
			   omit-xml-declaration? standalone version)
	  (put-string out newline)  ;; newline at the end of file
	  (close-port out)))
