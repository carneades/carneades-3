#| 
Contains useful utility functions for node selection via sxpath. 
Ported to ERR5RS / Larceny by Ray Racine
|#

;; Original author and license blurb
...
(library
 ...
 (export
  filter-and-map
  sxml:text
  sxml:node-parent sxml:attr sxml:attr-from-list sxml:content sxml:lookup
  sxml:name->ns-id sxml:ns-list sxml:ncname sxml:string->html sxml:sxml->xml sxml:sxml->html)
 ...
 (import
  ...
  (only (carneades lib xml ssax util)
	make-char-quotator)
  (only (carneades lib xml sxml info)
	name)
  (only (carneades lib xml sxml sxpathlib)
	nodeset? ntype?? ntype-names?? select-kids sxml:attr-list sxml:element? sxml:filter))
