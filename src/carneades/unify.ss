...
(module unify mzscheme
  ...
  (provide variable? identity unify* unify genvar rename-variables ground?)
  ...
  ; The hash-table can be modified.
  (define (rename-variables tbl trm)
    (cond ((variable? trm) 
           (hash-table-get tbl trm 
                           (lambda () 
                             (let ((v (genvar)))
                               (hash-table-put! tbl trm v) 
                               v))))
          ((pair? trm)
