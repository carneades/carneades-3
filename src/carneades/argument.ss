...
(library 
 (argument)
  ...
  (define-struct (ordinary-premise premise)())
  (define-struct (assumption premise)())
  (define-struct (exception premise)())
  ...
  (define (datum->argument sexp)
    ...
    (define (get-conclusion l)
      (let ((sexp (list:filter (lambda (sexp) (eq? (car sexp) 'conclusion))
                               l)))
        (if (null? sexp) 
  ...
  (define-struct context 
    (status        ; statement -> status table
     standard      ; statement -> standard
     compare))     ; argument * argument -> {-1, 0, 1}   
  
  (define default-context 
    (make-context (table:make-ordered statement-compare) 
  ...
  (define (status c s)
    (let ((v (table:lookup (statement-atom s) (context-status c) (lambda () 'stated))))
  ...
  (define (question context statements)
    (list:fold-right (lambda (s c) 
                       (make-context (table:insert (statement-atom s) 
                                                   'questioned 
                                                   (context-status c))
                                     (context-standard c) 
                                     (context-compare c)))
  ...
  (define-struct argument-graph
    (nodes        ; table: statement -> node
     arguments))  ; table: argument id -> argument 
  
  (define empty-argument-graph 
    (make-argument-graph (table:make-ordered statement-compare) 
                         (table:make-ordered compare:default-compare)))
  
  ; put-node: argument-graph node -> argument-graph 
  (define (put-node ag n)
    (make-argument-graph (table:insert (node-statement n) n (argument-graph-nodes ag))
                         (argument-graph-arguments ag)))
  
  ; get-node: argument-graph statement -> node | #f
  (define (get-node ag s)
    (table:lookup s (argument-graph-nodes ag) (lambda () #f)))
  ...
  (define (put-argument ag arg)
    ...
    (let ((ag1 (list:fold-right (lambda (p ag1)
      ...
      (make-argument-graph (argument-graph-nodes ag1)
                           (table:insert (argument-id arg) arg (argument-graph-arguments ag1)))))
  
  ; get-argument: argument-graph symbol -> argument | #f
  (define (get-argument ag id)
    (table:lookup id (argument-graph-arguments ag) (lambda () #f)))
  ...
  (define (schemes-applied ag s)
    (let ((n (get-node ag s))
      ...
      (if (not (node? n))
          ()
          (let ((pro-schemes  (list:fold-right f () (node-pro n)))
                (con-schemes (list:fold-right f () (node-con n))))
            (append pro-schemes con-schemes)))))
  
  
  ; accepted?: context statement -> boolean
  (define (accepted? c s)
    (let ((v (table:lookup (statement-atom s) (context-status c) (lambda () 'undecided))))
      (if (statement-positive? s)
          (eq? v 'accepted)
          (eq? v 'rejected))))
  ...
  (define (decided? c s)
    (member (table:lookup (statement-atom s) (context-status c) (lambda () 'undecided))
            '(rejected accepted)))
    
  ; questioned?: context statement -> boolean
  (define (questioned? c s)
    (eq? 'questioned (table:lookup (statement-atom s)
                                   (context-status c) (lambda () 'undecided))))
  
  ; stated?: context statement -> boolean
  (define (stated? c s)
    (eq? 'stated (table:lookup (statement-atom s) 
                               (context-status c) 
                               (lambda () 'undecided))))
  ...
  (define (questions ag c)
    (list:filter (lambda (s) (questioned? c s)) (statements ag)))
  ...
  (define (facts ag c)
    (map (lambda (s) 
           (if (rejected? c s)
               `(not ,s)
               s))
         (list:filter (lambda (s) (decided? c s)) (statements ag))))
  
  ; accepted-statements: context -> (list-of statement)
  (define (accepted-statements c)
    (table:keys (table:filter/value (lambda (value) (eq? value 'accepted)) (context-status c))))
  
  ; rejected-statements: context -> (list-of statement)
  (define (rejected-statements c)
    (table:keys (table:filter/value (lambda (value) (eq? value 'rejected)) (context-status c))))
  
  ; stated-statements: argument-graph context -> (list-of statement)
  (define (stated-statements ag c)
     (list:filter (lambda (s) (stated? c s)) (statements ag)))
  
  ; issues: argument-graph context -> (list-of statement)
  (define (issues ag c)
     (list:filter (lambda (s) (not (decided? c s))) (statements ag)))
  ...
  (define (relevant-statements ag g)
    (list:filter (lambda (s) (relevant? ag s g))
                 (statements ag))) 
  ...
  (define (assert* ag arg replace)
    (cond ((and (not replace) 
          ...
          ((not (cycle-free? arg ag))
           (printf "debug: cyclic argument ~v~n" (argument->datum arg))
  ...
  (define (assert ag args) 
    (list:fold-right (lambda (arg ag) (assert* ag arg #f))
                     ag 
                     args))
  ...
  (define (update ag args)
    (list:fold-right (lambda (arg ag) (assert* ag arg #t))
                     ag args))
  ...
  (define (best-argument? ag c n)
    (let* ((pro (list:filter (lambda (arg) (all-premises-hold? ag c arg))
                             (pro-arguments ag (node-statement n))))
           (con (list:filter (lambda (arg) (all-premises-hold? ag c arg))
                             (con-arguments ag (node-statement n))))
