...
(module argument-search mzscheme
  
  ; This  module applies argument generators to search for argument graphs in 
  ...
  (provide find-arguments find-best-arguments goal-state opposing-viewpoint state? 
  ...
  (define (instantiated-arguments state)
    (instantiate-argument-graph (state-arguments state)
  ...
  (define (questioned-assumptions assumptions context)
    (list:filter (lambda (s) (questioned? context s)) assumptions))
  ...
  (define (make-successor-state state new-subs arg)
    (let* ((conclusion (argument-conclusion arg))
           (assumptions (questioned-assumptions (map premise-statement 
                                                     (list:filter assumption? (argument-premises arg)))
                                                (state-context state)))
           (premises (append (map premise-statement 
                                  (list:filter ordinary-premise? (argument-premises arg)))
                             assumptions))
           (exceptions (map premise-statement 
                            (list:filter exception? (argument-premises arg))))
           (new-state
            (make-state (state-topic state)
                        ...
                        (question (state-context state) (list conclusion))
                        new-subs
                        ; extend argument graph with the new  
                        (assert (state-arguments state) (list arg)))))
      (if *debug* 
                                ...
                                (stream-flatmap 
                                 (lambda (clause) ; -> (stream-of (pair-of argument substitutions))
                                   ...
                                   (generator (car clause) ; first statement of clause
                                              state1))
                                 (apply stream (next-goals state1)))))))
    (lambda (node)
  ...
  (define (goal-state state)
    (let ((in (in? (state-arguments state) 
                            (state-context state)
                            (state-topic state))))
      (case (state-viewpoint state)
        ((pro) in)
