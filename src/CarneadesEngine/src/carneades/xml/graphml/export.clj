...
(defn- statement-nodes->graphml
  ...
  (reduce (fn [v stmt] 
	    (conj v [:node {:id (str (:id stmt))}
		     (if (nil? (:header stmt)) ""
			 [:data {:key "metadata"} (metadata->xml (:header (:header stmt)))])
		     (if (nil? (:atom stmt)) ""
...
(defn- argument-nodes->graphml
  ...
  (reduce (fn [v arg]
	    (-> (conj v [:node {:id (str (:id arg))}
			 (if (nil? (:header stmt)) ""
			     [:data {:key "metadata"} (metadata->xml (:header (:header arg)))])
			 [:data {:key "d6"}
			  [:y:ShapeNode
			   [:y:BorderStyle {:color "#000000" :type "line" :width "1.0"}]
			   [:y:NodeLabel {:alignment "center" :autoSizePolicy "content" 
		...
		((fn [v s] (apply conj v s))
		 (reduce (fn [v p] 
			   (conj v [:edge {:id (gensym "e")
					   :source (str (:id arg))
					   :target (str (literal-atom (:statement p)))}
				    [:data {:key "d10"}
				     [:y:LineStyle {:color "#000000" :type "line" :width "1.0"}]
				     [:y:Arrows {:source "none" 
						 :target (if (:positive p) "none" "dash")}]]]))
			 []
			 (:premises arg)))))
	  []
	  arg-nodes))
  ...
  (prx/prxml 
   [:graphml {:xmlns "http://graphml.graphdrawing.org/xmlns"
    ...
    (-> [:graph {:edgedefault "undirected" :id "G"}
	 [:data {:key "metadata"} (metadata->xml (:header ag))]]
