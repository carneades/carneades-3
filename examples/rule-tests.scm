(module rule-tests mzscheme
  ...
  (test/text-ui
    (test-suite
     ...
     (test-true "variable" (all-acceptable? '(bird ?x) (engine 20 1 null)))
     (test-true "rule with atomic body" (all-acceptable? '(money item1) (engine 20 1 null))) ; coins are money
     (test-true "conjunction" (all-acceptable? '(prior ?r1 ?r2) (engine 20 1 null)))
     (test-true "disjunction1" (all-acceptable? '(p3 a) (engine 20 1 null))) ; disjunction of atomic statements
     (test-true "disjunction2" (all-acceptable? '(p9 a) (engine 20 1 null))) ; disjunction of conjunctions
     (test-true "rule with exception" (all-acceptable? '(goods item1) (engine 20 1 null))) ; find pro argument
     (test-true "undercutter" (not (all-acceptable? '(goods item1) (engine 20 2 null)))) ; unless money exception
     (test-true "rebuttal"  (not (all-acceptable? '(goods item2) (engine 20 2 null)))) ; edible things are not goods
     (test-true "validity" (not (all-acceptable? '(convenient item1) (engine 20 2 '(valid))))) ; repealed rules are not valid
     (test-true "priorities" (not (all-acceptable? '(goods item2) (engine 20 2 '(priority))))) ; lex posterior
     ...
     (test-true "negative query" (all-acceptable? '(not (goods item2)) (engine 20 3 null))) 
     (test-true "exclusionary rule" (not (all-acceptable? '(flies Tweety) (engine 20 2 '(excluded)))))
     (test-true "applies" (all-acceptable? '(applies ?r (goods ?x)) (engine 20 1 null))) 
     ; to do: test negative conditions and exceptions
     ; to do: test rules with negative conclusions
     (test-true "eval1" (all-acceptable? '(p11 ?x) (engine 20 1 null))) ; reverse a list
     (test-true "eval2" (all-acceptable? '(taxable-income Sam ?x) (engine 20 1 null))) ; calculations
